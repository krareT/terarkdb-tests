...
|选项(Option)|说明|
|------------|---------------|
|--action=|load 或者 run<br/>load 是只写，以最快的速度把数据写入 DB，并执行 compact, compact 结束程序就退出<br/>run 可以执行读写混合测试，程序会一直运行，直到用户按下 `Ctrl + C`|
...
|--use\_universal\_compaction=|1 或 0，1 表示使用 univeral compaction，0表示使用 Level based compaction，默认为1|
